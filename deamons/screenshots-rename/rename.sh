...
if printf '%s\n' "$old_filename" | grep -vE '^Screenshot [0-9\-]+ at [0-9\.]+( \([0-9]+\))?\.png$' >/dev/null; then
    exit 0
fi

date="$(printf '%s\n' "$(basename "$file")" | sed -E 's~^Screenshot ([0-9]{4})-([0-9]{2})-([0-9]{2}) at ([0-9]{2})\.([0-9]{2})\.([0-9]{2})( \([0-9]+\))?\.png$~\1-\2-\3_\4-\5-\6~')"
...
if printf '%s\n' "$old_filename" | grep -E '^Screenshot [0-9\-]+ at [0-9\.]+ \([0-9]+\)\.png$' >/dev/null; then
    index="$(printf '%s\n' "$old_filename" | sed -E 's~^Screenshot [0-9\-]+ at [0-9\.]+ \(([0-9]+)\)\.png$~\1~')"
